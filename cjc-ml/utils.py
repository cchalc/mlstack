...
def _get_ml_config_value(env, key):
    ...
    conf_file_path = os.path.join(
        os.pardir, "cjc-ml", "terraform", "output", f"{env}.json"
    )
    try:
        ...
        raise RuntimeError(
            ...
            f"cjc-ml have been deployed to {env} (see cjc-ml"
...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"cjc-ml-model{resource_name_suffix}"
    else:
        # Read ml model name from cjc-ml/terraform
        return _get_ml_config_value(env, "cjc-ml_model_name")
